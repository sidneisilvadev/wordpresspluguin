        ...
        try {
            ...
            foreach ($body['data'] as $model) {
                if (isset($model['id']) && isset($model['name'])) {
                    ...
                    // Se não for gratuito, adiciona indicador (Pago)
                    if (!$is_free) {
                        $display_name .= ' (Pago)';
                    }
                    
                    // Organiza os modelos: primeiro os gratuitos, depois os pagos
                    if ($is_free) {
                        $models[$model['id']] = $display_name;
                    } else {
            ...
            if (empty($models)) {
                throw new Exception('No models found in OpenRouter API response');
            }

            // Ordena os modelos para colocar os gratuitos primeiro
            $free_models = array();
            $paid_models = array();
            ...
            // Combina os arrays mantendo os gratuitos primeiro
            return array_merge($free_models, $paid_models);
